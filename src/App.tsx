...
export default function App() {
  ...
  const [podcastAnnual, setPodcastAnnual] = useState(false);
    ...
    <div className="font-sans">
      ...
      <Section id="reels" className="bg-white">
        ...
        <div className="max-w-xl mx-auto">
          <FadeIn delay={0.1}>
            <div className="bg-black text-white p-10 rounded-[2.5rem] shadow-2xl relative">
              ...
              <h3 className="text-xl font-bold mb-2">Reels Plan</h3>
              <div className="flex items-baseline gap-1 mb-2">
                <span className="text-5xl font-bold">${reelsAnnual ? '169' : '199'}</span>
                <span className="text-white/40">/ mes</span>
              </div>
              {reelsAnnual && <p className="text-emerald-400 text-sm font-bold mb-6">$2.028 /año — Ahorrás $360</p>}
              {!reelsAnnual && <div className="mb-6" />}
              <ul className="space-y-4 mb-10">
                {[
                  "5 Reels editados al mes",
                  "Guiones estratégicos con hooks",
                  "Subtítulos animados",
              ...
              <button onClick={() => handleScheduleClick('reels_plan')} className="w-full py-4 rounded-2xl bg-white text-black font-bold hover:bg-white/90 transition-all shadow-lg shadow-white/5 cursor-pointer">
                Empezar ahora
              </button>
            </div>
          </FadeIn>
      ...
      <Section id="precios" className="bg-neutral-50">
        <div className="text-center mb-16">
          <FadeIn>
            <h2 className="text-4xl md:text-6xl font-bold tracking-tight mb-6">Plan Podcast</h2>
            <p className="text-xl text-black/60 max-w-2xl mx-auto">Pensado para quienes están construyendo autoridad desde cero. $200 efectivo por episodio.</p>
          </FadeIn>
          <FadeIn delay={0.1}>
            <div className="flex items-center justify-center gap-4 mt-10">
              <span className={`font-semibold transition-colors ${!podcastAnnual ? 'text-black' : 'text-black/40'}`}>Mensual</span>
              <button
                onClick={() => setPodcastAnnual(!podcastAnnual)}
                className={`relative w-14 h-7 rounded-full transition-colors cursor-pointer ${podcastAnnual ? 'bg-emerald-500' : 'bg-black/10'}`}
              >
                <div className={`absolute top-0.5 left-0.5 w-6 h-6 bg-white rounded-full shadow transition-transform ${podcastAnnual ? 'translate-x-7' : ''}`} />
              </button>
              <span className={`font-semibold transition-colors ${podcastAnnual ? 'text-black' : 'text-black/40'}`}>Anual</span>
              {podcastAnnual && <span className="text-emerald-600 text-sm font-bold">Ahorrá 15%</span>}
            </div>
          </FadeIn>
        </div>
        ...
        <FadeIn>
          <div className="mb-8 bg-black text-white rounded-[2.5rem] p-10 md:p-12 relative overflow-hidden shadow-2xl max-w-4xl mx-auto">
            ...
            <div className="relative z-10 grid md:grid-cols-2 gap-10 items-center">
              <div>
                <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-500 text-white rounded-full text-[10px] font-bold uppercase tracking-widest mb-5">
                  <ShieldCheck size={14} /> Más popular
                </div>
                <h3 className="text-3xl md:text-5xl font-bold mb-4 tracking-tight">Growth</h3>
                <p className="text-white/70 text-lg mb-6">
                  Para fundadores que están comenzando su proyecto y quieren un sistema de autoridad consistente.
                </p>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-5xl md:text-6xl font-bold">${podcastAnnual ? '679' : '799'}</span>
                  <span className="text-white/40 text-xl">/ mes</span>
                </div>
                {podcastAnnual && <p className="text-emerald-400 text-sm font-bold">$8.148 /año — Ahorrás $1.440</p>}
                {!podcastAnnual && <p className="text-white/40 text-sm">$200 efectivo por episodio.</p>}
              </div>
              <div>
                <ul className="space-y-4 mb-8">
                  {[
                    "4 Episodios al mes",
                    "5–7 Clips verticales por episodio",
                ...
                <button onClick={() => handleScheduleClick('pricing_growth')} className="w-full py-4 rounded-2xl bg-emerald-500 text-white font-bold hover:bg-emerald-400 transition-all shadow-lg shadow-emerald-500/20 cursor-pointer">
                  Aplicar al plan Growth
                </button>
              </div>
            </div>
          </div>
        ...
        {/* Piloto Banner */}
        <FadeIn delay={0.1}>
          <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-black/5 flex flex-col md:flex-row items-center justify-between gap-4 max-w-4xl mx-auto">
            <div className="flex-1">
              ...
              <p className="text-black/60 text-sm md:text-base">1 Episodio completo + 5–7 clips estratégicos + show notes. Si continuás, se descuenta del primer mes.</p>
            </div>
